...
async def doge_register(account: str, coin: str, user_server: str = 'DISCORD') -> str:
    COIN_NAME = coin.upper()
    naming = "bountybot_" + account
    if user_server == "TELEGRAM":
        naming = "telebountybot_" + account
    payload = f'"{naming}"'
...
def get_min_bounty(coin: str):
    return getattr(config,"coin"+coin,config.coinWRKZ).min_bounty


def get_max_bounty(coin: str):
    return getattr(config,"coin"+coin,config.coinWRKZ).max_bounty
